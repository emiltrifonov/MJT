...
public record SemesterPlan (UniversitySubject[] subjects, SubjectRequirement[] subjectRequirements, int minimalAmountOfCredits) {
    public SemesterPlan {
        if (subjects == null) {
            throw new IllegalArgumentException("Null subjects in SemesterPlan constructor.");
        }
        else if (subjectRequirements == null) {
            throw new IllegalArgumentException("Null subjectRequirements in SemesterPlan constructor.");
        }
        else if (minimalAmountOfCredits < 0) {
            throw new IllegalArgumentException("Negative minimalAmountOfCredits in SemesterPlan constructor.");
        }
    }
}